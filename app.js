...
const DEFAULT = {
  ...
  activePlace:1, places:[
    {id:1,name:'Sderot, Israel',lat:31.5235,lon:34.5931,bortle:8,sqm:null,window:{enabled:false,left:90,right:270,bottom:25,top:85}},
    {id:2,name:'Shittim / Negev',lat:30.33,lon:34.95,bortle:3,sqm:null,window:{enabled:false,left:0,right:360,bottom:15,top:90}}
  ],
  activeRig:1, rigs:[
...
let sensorStep=0, sensorHeading=null, sensorAltitude=null;
const SENSOR_STEPS=[
  {key:'left',title:'Point to the LEFT edge',instruction:'Aim the back of your phone at the left edge of the opening. This captures compass azimuth.',value:'heading'},
  {key:'right',title:'Point to the RIGHT edge',instruction:'Aim at the right edge of the opening. The window may cross north (0°); that is supported.',value:'heading'},
  {key:'bottom',title:'Point to the BOTTOM edge',instruction:'Aim at the lowest usable line above roofs, trees or walls. This captures altitude.',value:'altitude'},
  {key:'top',title:'Point to the TOP edge',instruction:'Aim at the highest visible line of the opening. This captures altitude.',value:'altitude'}
];
...
function skyWindow(){return place().window||{enabled:false,left:0,right:360,bottom:rig().minAlt,top:90}}
...
function bind(){
  ...
  $('calibrateWindow').onclick=startWindowCalibration;$('cancelSensor').onclick=stopWindowCalibration;$('captureSensor').onclick=captureWindowBoundary;
...
function insideWindowAz(az){const w=skyWindow();if(!w.enabled)return true;return w.left<=w.right?(az>=w.left&&az<=w.right):(az>=w.left||az<=w.right)}
function insideSkyWindow(az,alt){const w=skyWindow();return insideWindowAz(az)&&(!w.enabled||(alt>=w.bottom&&alt<=w.top))}
...
function evaluateTarget(o,n){
  const hs=hoursForNight(n),fr=framing(o);let rows=hs.map(h=>{const pos=azAlt(h.time,o),moon=moonAt(h.time),sep=separation(pos,moon),min=Math.max(rig().minAlt,o.minAlt),inside=insideSkyWindow(pos.az,pos.alt),altQ=clamp((pos.alt-min)*3.2+35),moonFit=filterFit(o,n.illum.fraction,sep),wq=weatherQuality(h),score=(pos.alt<min||!inside)?0:Math.round(wq*.46+altQ*.25+moonFit*.19+fr.score*.10);return{...h,...pos,moonAlt:moon.alt,moonSep:sep,score,minAlt:min,insideWindow:inside}});
  const geometric=rows.filter(x=>x.insideWindow&&x.alt>=x.minAlt),usable=geometric.filter(x=>x.score>=60),max=rows.reduce((a,b)=>a.alt>b.alt?a:b,rows[0]||{alt:0}),rankRows=[...geometric].sort((a,b)=>b.score-a.score).slice(0,Math.min(3,geometric.length)),rank=rankRows.length?Math.round(rankRows.reduce((s,x)=>s+x.score,0)/rankRows.length):0,bestInside=[...geometric].sort((a,b)=>b.score-a.score)[0]||null;
  return{target:o,rows,score:rank,hours:usable.length,geometryHours:geometric.length,maxAlt:max.alt||0,transit:max.time,framing:fr,best:bestWindow(usable),bestInside,reason:targetReason(o,geometric,usable,bestInside,n)};
}
function targetReason(o,geometric,usable,best,n){if(!geometric.length)return'Outside the measured sky window or below the minimum altitude';if(usable.length)return`${usable.length} weather-usable hour${usable.length===1?'':'s'} inside the window`;const reasons=[];if(best){if(best.cloud>state.limits.cloud)reasons.push(`${Math.round(best.cloud)}% cloud`);if(best.gust>rig().wind)reasons.push(`${Math.round(best.gust)} km/h gusts`);if(best.precipProb>20)reasons.push(`${Math.round(best.precipProb)}% rain risk`);if(best.temp-best.dew<state.limits.dew)reasons.push('dew risk');if(filterFit(o,n.illum.fraction,best.moonSep)<55)reasons.push('filter/Moon mismatch')}return`Inside geometrically, but no hour reaches 60/100${reasons.length?' — best hour limited by '+reasons.join(', '):` — best combined hour is ${best?.score||0}/100`}`}
...
function renderTargets(){if(!nights.length)return;const q=$('targetSearch').value.toLowerCase(),type=$('targetType').value,bm=$('bookmarksOnly').checked;evaluations=nights[0].evals.filter(e=>(!q||e.target.name.toLowerCase().includes(q))&&(type==='all'||e.target.type===type)&&(!bm||state.bookmarks.includes(e.target.name)));$('targetList').innerHTML=evaluations.map(e=>{const windowRows=e.rows.filter(x=>x.insideWindow&&x.alt>=x.minAlt),entry=windowRows[0]?.time,exit=windowRows.length?new Date(windowRows.at(-1).time.getTime()+3600000):null;return`<article class="target-card card" data-id="${e.target.id}"><div><span class="pill">${e.target.type}</span><h3>${e.target.name}</h3><div class="target-meta">${e.framing.label} · rank ${e.score}/100 · ${e.hours}h weather-usable · max ${Math.round(e.maxAlt)}°</div><div class="target-meta ${entry?'inside-window':'outside-window'}">${entry?`Inside your sky window ${siteFmt(entry)}–${siteFmt(exit)}`:'Not inside your sky window tonight'}</div><div class="target-meta">${e.reason}</div></div><div><button class="bookmark" data-bookmark="${e.target.id}">${state.bookmarks.includes(e.target.name)?'★':'☆'}</button><div class="target-score">${e.score}</div></div></article>`}).join('')||'<article class="card muted">No matching targets.</article>';document.querySelectorAll('.target-card').forEach(c=>c.onclick=e=>{if(e.target.closest('.bookmark'))return;showTarget(+c.dataset.id)});document.querySelectorAll('.bookmark').forEach(b=>b.onclick=()=>toggleBookmark(+b.dataset.bookmark))}
...
function showTarget(id){const e=nights[0].evals.find(x=>x.target.id===id),w=skyWindow(),windowRows=e.rows.filter(x=>x.insideWindow&&x.alt>=x.minAlt),entry=windowRows[0]?.time,exit=windowRows.length?new Date(windowRows.at(-1).time.getTime()+3600000):null;state.selectedTarget=e.target.name;save();$('targetDetail').innerHTML=`<p class="eyebrow">${e.target.type}</p><h2>${e.target.name}</h2><p class="muted">Rank ${e.score}/100 · ${e.framing.label} · ${round(e.hours,1)} weather-usable hours · transit ${siteFmt(e.transit)} at ${Math.round(e.maxAlt)}°.</p><p class="${e.hours?'inside-window':'outside-window'}">${e.reason}</p><div class="data-grid"><div><span>Inside measured window</span><strong>${entry?siteFmt(entry)+'–'+siteFmt(exit):'Never tonight'}</strong></div><div><span>Apparent size</span><strong>${e.target.w}' × ${e.target.h}'</strong></div><div><span>Your FOV</span><strong>${round(optics().fovW,2)}° × ${round(optics().fovH,2)}°</strong></div><div><span>Best weather window</span><strong>${e.best?siteFmt(e.best.start)+'–'+siteFmt(e.best.end):'None ≥60'}</strong></div></div><button class="primary wide" id="selectTarget">Use as selected target</button>`;$('targetDialog').showModal();setTimeout(()=>{$('selectTarget').onclick=()=>{$('targetDialog').close();renderAll();toast('Target selected')};if(targetChart)targetChart.destroy();const azVisible=e.rows.map(x=>insideWindowAz(x.az));targetChart=new Chart($('targetChart'),{type:'line',data:{labels:e.rows.map(x=>siteFmt(x.time)),datasets:[{label:'Window bottom',data:e.rows.map((x,i)=>azVisible[i]&&w.enabled?w.bottom:null),borderColor:'#34d399',backgroundColor:'#34d39922',borderDash:[3,3],pointRadius:0,spanGaps:false},{label:'Available sky window',data:e.rows.map((x,i)=>azVisible[i]&&w.enabled?w.top:null),borderColor:'#34d399',backgroundColor:'#34d39922',fill:'-1',borderDash:[3,3],pointRadius:0,spanGaps:false},{label:'Target altitude',data:e.rows.map(x=>Math.max(0,x.alt)),borderColor:'#38bdf8',backgroundColor:'#38bdf822',pointRadius:0,borderWidth:3},{label:'Minimum target altitude',data:e.rows.map(x=>x.minAlt),borderColor:'#fb7185',borderDash:[3,3],pointRadius:0},{label:'Moon altitude',data:e.rows.map(x=>Math.max(0,x.moonAlt)),borderColor:'#e2e8f0',borderDash:[5,5],pointRadius:0}]},options:{responsive:true,maintainAspectRatio:false,interaction:{mode:'index',intersect:false},scales:{x:{ticks:{color:'#8ca0b9'}},y:{min:0,max:90,ticks:{color:'#8ca0b9'}}},plugins:{legend:{labels:{color:'#8ca0b9',filter:item=>item.text!=='Window bottom'}},tooltip:{callbacks:{afterBody:items=>{const x=e.rows[items[0].dataIndex];return[`Azimuth ${Math.round(x.az)}°`,x.insideWindow?'Inside measured window':'Outside measured window',`Combined hour score ${x.score}/100`]}}}}}})},30)}
...
function addPlace(e){e.preventDefault();const p={id:Date.now(),name:$('placeName').value,lat:+$('placeLat').value,lon:+$('placeLon').value,bortle:+$('placeBortle').value||null,sqm:+$('placeSqm').value||null,window:{enabled:false,left:0,right:360,bottom:20,top:90}};state.places.push(p);state.activePlace=p.id;save();e.target.reset();fillPlaces();refresh()}
function useGPS(){navigator.geolocation?.getCurrentPosition(pos=>{const p={id:Date.now(),name:'GPS observing site',lat:pos.coords.latitude,lon:pos.coords.longitude,bortle:null,sqm:null,window:{enabled:false,left:0,right:360,bottom:20,top:90}};state.places.push(p);state.activePlace=p.id;save();fillPlaces();refresh();toast('GPS site added')},()=>toast('Location permission was not available'),{enableHighAccuracy:true,timeout:10000})}

function fillRig(){$('rigSelect').innerHTML=state.rigs.map(r=>`<option value="${r.id}" ${r.id===state.activeRig?'selected':''}>${r.name}</option>`).join('');const r=rig();$('rigName').value=r.name;$('focalLength').value=r.focal;$('aperture').value=r.aperture;$('sensorWidth').value=r.sensorW;$('sensorHeight').value=r.sensorH;$('pixelSize').value=r.pixel;$('filter').value=r.filter;$('mount').value=r.mount;$('windLimit').value=r.wind;$('minAlt').value=r.minAlt;const w=skyWindow();$('winLeft').value=w.left;$('winRight').value=w.right;$('winBottom').value=w.bottom;$('winTop').value=w.top;$('windowEnabled').checked=w.enabled;renderWindowSummary();const o=optics();$('opticsSummary').innerHTML=`<div><span>Focal ratio</span><strong>f/${round(o.fRatio,1)}</strong></div><div><span>Image scale</span><strong>${round(o.scale,2)}″/px</strong></div><div><span>Field of view</span><strong>${round(o.fovW,2)}° × ${round(o.fovH,2)}°</strong></div><div><span>Sampling</span><strong>${o.scale>2?'Wide / undersampled':o.scale<.6?'Fine / seeing-sensitive':'Balanced'}</strong></div>`}
...
function saveHorizon(){const left=normalizeAz(+$('winLeft').value),right=normalizeAz(+$('winRight').value),bottom=+$('winBottom').value,top=+$('winTop').value;if(top<=bottom){toast('Top altitude must be higher than bottom');return}place().window={enabled:$('windowEnabled').checked,left,right,bottom,top};save();renderWindowSummary();renderAll();toast('Available sky window saved for this site')}
function normalizeAz(x){return((x%360)+360)%360}
function renderWindowSummary(){const w={enabled:$('windowEnabled').checked,left:+$('winLeft').value,right:+$('winRight').value,bottom:+$('winBottom').value,top:+$('winTop').value},span=w.left<=w.right?w.right-w.left:360-w.left+w.right;$('windowSummary').innerHTML=w.enabled?`Active window: <strong>${Math.round(span)}° wide</strong>, azimuth ${Math.round(w.left)}° → ${Math.round(w.right)}°, altitude ${Math.round(w.bottom)}° → ${Math.round(w.top)}°. Targets are accepted only while their centre is inside this box.`:'Window filtering is currently disabled.'}

async function startWindowCalibration(){
  if(!('DeviceOrientationEvent' in window)){toast('Orientation sensors are not available on this device');return}
  if(typeof DeviceOrientationEvent.requestPermission==='function'){
    try{if(await DeviceOrientationEvent.requestPermission()!=='granted'){toast('Sensor permission was not granted');return}}catch{toast('Could not access phone sensors');return}
  }
  sensorStep=0;sensorHeading=null;sensorAltitude=null;
  window.addEventListener('deviceorientation',readOrientation,true);$('sensorDialog').showModal();renderSensorStep();
}
function readOrientation(e){
  if(e.alpha!=null)sensorHeading=normalizeAz(Number.isFinite(e.webkitCompassHeading)?e.webkitCompassHeading:360-e.alpha);
  if(e.beta!=null){let beta=e.beta;if(beta>90)beta=180-beta;if(beta<-90)beta=-180-beta;sensorAltitude=clamp(90-Math.abs(beta),-10,90)}
  renderSensorReading();
}
function renderSensorStep(){const s=SENSOR_STEPS[sensorStep];$('sensorStepTitle').textContent=s.title;$('sensorInstructions').textContent=s.instruction;$('captureSensor').textContent=`Capture ${s.key} boundary`;$('sensorProgress').innerHTML=SENSOR_STEPS.map((_,i)=>`<i class="${i<sensorStep?'done':i===sensorStep?'current':''}"></i>`).join('');renderSensorReading()}
function renderSensorReading(){const s=SENSOR_STEPS[sensorStep];if(!s)return;const main=s.value==='heading'?sensorHeading:sensorAltitude,secondary=s.value==='heading'?`Altitude ${sensorAltitude==null?'—':Math.round(sensorAltitude)+'°'}`:`Compass ${sensorHeading==null?'—':Math.round(sensorHeading)+'°'}`;$('sensorValue').textContent=main==null?'—°':`${Math.round(main)}°`;$('sensorSecondary').textContent=secondary}
function captureWindowBoundary(){const s=SENSOR_STEPS[sensorStep],value=s.value==='heading'?sensorHeading:sensorAltitude;if(value==null){toast('Waiting for a valid sensor reading');return}$('win'+s.key[0].toUpperCase()+s.key.slice(1)).value=Math.round(value);sensorStep++;if(sensorStep>=SENSOR_STEPS.length){$('windowEnabled').checked=true;stopWindowCalibration();renderWindowSummary();toast('Four boundaries captured — review and save')}else renderSensorStep()}
function stopWindowCalibration(){window.removeEventListener('deviceorientation',readOrientation,true);if($('sensorDialog').open)$('sensorDialog').close()}
