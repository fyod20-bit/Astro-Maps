...
function insideSkyWindow(az,alt){const w=skyWindow();if(!w.enabled)return true;const azInside=w.left<=w.right?(az>=w.left&&az<=w.right):(az>=w.left||az<=w.right);return azInside&&alt>=w.bottom&&alt<=w.top}
...
function renderTargets(){if(!nights.length)return;const q=$('targetSearch').value.toLowerCase(),type=$('targetType').value,bm=$('bookmarksOnly').checked;evaluations=nights[0].evals.filter(e=>(!q||e.target.name.toLowerCase().includes(q))&&(type==='all'||e.target.type===type)&&(!bm||state.bookmarks.includes(e.target.name)));$('targetList').innerHTML=evaluations.map(e=>{const windowRows=e.rows.filter(x=>x.insideWindow&&x.alt>=x.minAlt),entry=windowRows[0]?.time,exit=windowRows.length?new Date(windowRows.at(-1).time.getTime()+3600000):null;return`<article class="target-card card" data-id="${e.target.id}"><div><span class="pill">${e.target.type}</span><h3>${e.target.name}</h3><div class="target-meta">${e.framing.label} · ${e.hours}h weather-usable · max ${Math.round(e.maxAlt)}°</div><div class="target-meta ${entry?'inside-window':'outside-window'}">${entry?`Inside your sky window ${siteFmt(entry)}–${siteFmt(exit)}`:'Not inside your sky window tonight'}</div></div><div><button class="bookmark" data-bookmark="${e.target.id}">${state.bookmarks.includes(e.target.name)?'★':'☆'}</button><div class="target-score">${e.score}</div></div></article>`}).join('')||'<article class="card muted">No matching targets.</article>';document.querySelectorAll('.target-card').forEach(c=>c.onclick=e=>{if(e.target.closest('.bookmark'))return;showTarget(+c.dataset.id)});document.querySelectorAll('.bookmark').forEach(b=>b.onclick=()=>toggleBookmark(+b.dataset.bookmark))}
